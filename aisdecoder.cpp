...
AisData AisDecoder::parseAisMessage(const QString &nmea) {
    ...
    if (result.messageType < 1 || result.messageType > 3)
        return result;
    ...
    int longitudeRaw = binaryToInt(bin, 61, 28, true);
    int latitudeRaw = binaryToInt(bin, 89, 27, true);
    result.cog = binaryToInt(bin, 116, 12);
    result.heading = binaryToInt(bin, 128, 9);
    result.timestamp = binaryToInt(bin, 137, 6);
    result.latitude = decodeLatitude(latitudeRaw);
    result.longitude = decodeLongitude(longitudeRaw);
