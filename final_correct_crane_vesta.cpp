...
int main() {
    cout << "=== FINAL CORRECT Type 5 for CRANE VESTA ===" << endl;
    cout << "Using same structure that works for TEST/BOAT" << endl;

    string bitstream;

    // Type 5 header structure yang sudah terbukti bekerja
    bitstream += "000101";  // Type 5
    bitstream += "00";      // Repeat
    bitstream += "000000011011110001010101101101";  // MMSI 123456789
    bitstream += "00";      // AIS Version
    bitstream += "000000000000000000000000000000";  // IMO

    // Callsign: Empty (7 chars)
    string callsignEncoded = encode6bitString("", 7);
    bitstream += callsignEncoded;

    // VESSEL NAME: "CRANE VESTA" (20 chars)
    ...
    cout << "Vessel starts at bit: " << (bitstream.length() - vesselEncoded.length()) << endl;
    ...
    cout << "\n=== FINAL CORRECT NMEA FOR CRANE VESTA ===" << endl;
    cout << nmea << endl;

    cout << "\nExpected decoder result:" << endl;
    cout << "Type: 5 (Ship Static and Voyage Related Data)" << endl;
